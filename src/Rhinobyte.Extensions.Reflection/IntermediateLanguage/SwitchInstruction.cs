...
namespace Rhinobyte.Extensions.Reflection.IntermediateLanguage
{
	...
	public sealed class SwitchInstruction : InstructionBase
	{
		internal SwitchInstruction(int index, int offset, OpCode opcode, IReadOnlyCollection<int> targetOffsets)
			: base(index, offset, opcode, opcode.Size + OpCodeHelper.GetOperandSize(opcode.OperandType) + (targetOffsets?.Count ?? 0 * 4))
		{
			...
			TargetOffsets = targetOffsets ?? throw new ArgumentNullException(nameof(targetOffsets));
		}
